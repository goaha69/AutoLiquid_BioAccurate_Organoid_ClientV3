...
const loadDictData = async () => {
  try {
    ...
    // 设置默认液体范围
    if (liquidRanges.value.length > 0 && attributeData.liquidRange === 0) {
      attributeData.liquidRange = liquidRanges.value[0].code
    }
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await exp_liquid_add(submitData)
    if (res.success) {
