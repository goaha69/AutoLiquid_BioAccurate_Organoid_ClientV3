<template>
  <a-modal title="编辑数据:" :width="900" :open="visible" :confirmLoading="confirmLoading" @ok="handleSubmit"
    @cancel="handleCancel">
      ...
      <a-form :form="form">
        ...
        <a-form-item label="描述" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input placeholder="请输入描:" style="width: 100%"
            v-decorator="['columnDescription', { rules: [{ required: true, message: '请输入描述!' }] }]" ></a>
        </a-form-item>
        <a-form-item label="主键" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-select v-decorator="['isPrimarykey', { rules: [{ required: true, message: '请选择是否为主'}] }]"
        ...
        <a-form-item label="长度" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input  disabled="!hasLength" placeholder="请输入长 : " v-decorator="['length', validateLengthRule]" ></a>
        </a-form-item>
        <a-form-item label="保留小数" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input  disabled="!hasDecimalDigits" placeholder="请输入位 : " v-decorator="['decimalDigits']" ></a>
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
export default {
  data() {
    return {
      ...
      validateLengthRule: {
        rules: [
          {required: this.lengthIsRequire, message: '请输入字段长'},
          {
