<template>
  <a-modal title="编辑菜单" :width="1000" :open="visible" :confirmLoading="confirmLoading" @ok="handleSubmit"
            ...
            <div v-show="componentShow">
              <a-form-item  labelCol="labelCol" : wrapperCol="wrapperCol" hasFeedback>
                ...
                <a-input placeholder="请输入前端组:" :disabled="componentDisabled" prop="component"
                  v-decorator="['component', { rules: [{ required: componentRequired, message: '请输入前端组'}] }]" ></a>
              </a-form-item>
            </div>
          </a-col>
          <a-col  md="12" : sm="24">
        ...
        <a-row :gutter="24">
          ...
          <a-col  md="12" : sm="24">
            <a-form-item  labelCol="labelCol" : wrapperCol="wrapperCol" label="是否可见">
              <a-switch id="visible" checkedChildren=":" unCheckedChildren="'" v-decorator="['visible', { valuePropName: 'checked' }]" ></a>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row>
    ...
    <a-modal :width="850" :open="visibleIcon" @cancel="handleCancelIcon" footer="" :mask="false" :closable="false"
      :destroyOnClose="true">
      <icon-selector v-model="currentSelectedIcon" @change="handleIconChange" ></icon>
    </a-modal>
  </a-modal>
</template>
