        ...
        <a-row :gutter="24">
          <a-col :md="12" :sm="24">
            <a-form-item label="所属应用" name="application">
              <a-select v-model:value="formState.application" placeholder="请选择应用分类" disabled>
...
const rules = reactive({
  ...
  component: [{ required: false, message: '请输入前端组件!' }],
  router: [{ required: false, message: '请输入路由地址!' }],
  permission: [{ required: false, message: '请输入权限标识!' }],
  link: [{ required: false, message: '请输入链接地址!' }],
});
...
const edit = (record) => {
  ...
  // Load menu tree for the application
  getMenuTree({ application: record.application }).then(res => {
    menuTreeData.value = [{ id: '0', name: '顶级', children: res.data }];
  });

  // Using nextTick to ensure the form is rendered before setting values
  nextTick(() => {
    Object.assign(formState, record);
    formState.visible = record.visible === 'Y';
    formState.keepAlive = record.keepAlive === 'Y';
...
watch(() => formState.openType, (newOpenType) => {
    ...
    if(newOpenType === 'IFRAME' && formState.component !== 'Iframe') {
...
const handleSubmit = () => {
  formRef.value.validate().then(() => {
    ...
    params.keepAlive = params.keepAlive ? 'Y' : 'N';
    sysMenuEdit(params).then(res => {
      if (res.success) {
