<template>
  <a-modal title="编辑用户" :width="900" :open="visible" :confirm-loading="confirmLoading" @ok="handleSubmit" @cancel="handleCancel">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="12" :sm="24">
            <a-form-item label="生日" :label-col="labelCol" :wrapper-col="wrapperCol">
              <a-date-picker v-model:value="formState.birthday" style="width: 100%" placeholder="请选择生日" value-format="YYYY-MM-DD" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="24">
...
const edit = (record) => {
  ...
  Promise.all([getOrgTree(), sysPosList(), sysUserDetail({ id: record.id })]).then(([orgRes, posRes, userRes]) => {
    ...
    Object.assign(formState, userData, {
      ...
      sysEmpParam: userData.sysEmpParam || { orgId: undefined, jobNum: '', posIdList: [], extIds: [] }
    });
    
    if (formState.sysEmpParam.extIds && Array.isArray(formState.sysEmpParam.extIds)) {
        formState.sysEmpParam.extIds = formState.sysEmpParam.extIds.map(item => ({...item, key: Date.now() + Math.random()}));
    } else {
...
const addExt = () => {
    if (!formState.sysEmpParam.extIds) {
        formState.sysEmpParam.extIds = [];
    }
...
const handleSubmit = async () => {
  try {
    ...
    if(params.sysEmpParam.extIds) {
        params.sysEmpParam.extIds.forEach(item => delete item.key);
    }
    
    const res = await sysUserEdit(params);
    if (res.success) {
