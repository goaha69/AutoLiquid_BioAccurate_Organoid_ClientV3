buttonvalidateToNextPassword<template>
  <a-modal title="用户增加"  :width="900" :open="visible" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="handleCancel">
        ...
        <a-col  md="12" : sm="24">
          <a-form :form="form">
            <a-form-item label="密码" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
              <a-input placeholder="请输入密码 type="password" v-decorator="['password', {rules: [{required: true, min: 5, message: '请输入至少五个字符的密码'},{ validator: validateToNextPassword, },]}]" ></a>
            </a-form-item>
          </a-form>
        </a-col>
        <a-col  md="12" : sm="24">
          <a-form :form="form">
            <a-form-item label="重复密码" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
              <a-input placeholder="请再次输入密码 type="password" v-decorator="['confirm', {rules: [{required: true, message: '请再次输入密码!'}, { validator: compareToFirstPassword, }]}]" ></a>
            </a-form-item>
          </a-form>
        </a-col>
      </a-row>
            ...
            <a-table size="middle" :columns="columns" :dataSource="data" :pagination="false" :loading="memberLoading" >
              <template v-for="(col,index) in ['extOrgId','extPosId']" #[col]="{ text, record }">
                <template v-if="index == 0" >
                  <a-tree-select :key="col" v-if="record.editable"  :treeData="orgTree" style="width: 100%"  placeholder="请选择附属机构"  treeDefaultExpandAll @change="e => handleChange(e,record.key,col)" >
                    <template #title="{ id }">
                      <span>{{ id }}</span>
                    </template>
                  </a-tree-select>
                ...
                <template v-if="index == 1">
                  <a-select  key="col"  v-if="record.editable" style="width : 100%"  placeholder="请选择附属职位"  @change="e => handleChange(e,record.key,col)" has-feedback >
