...
export const generator = (routerMap, parent) => {
  return routerMap.map(item => {
    ...
    const currentRouter = {
      ...
      // 该路由对应页面的 组件，采用Vue2的简单方式处理，但适配Vue3
      component: (constantRouterComponents[item.component || item.key]) || 
        markRaw(defineAsyncComponent(() => {
          console.log('🔍 开始加载组件:', item.component || item.key)
          ...
          // 简化路径处理逻辑：先尝试完整路径，再尝试index.vue
          try {
            if (item.component && item.component.includes('/')) {
              return import(`@/views/${item.component}.vue`)
                .catch(() => import(`@/views/${item.component}/index.vue`))
                .catch(() => import('@/views/system/exception/404.vue'))
            } else if (item.component) {
              return import(`@/views/${item.component}/index.vue`)
                .catch(() => import(`@/views/${item.component}.vue`))
                .catch(() => import('@/views/system/exception/404.vue'))
            } else {
