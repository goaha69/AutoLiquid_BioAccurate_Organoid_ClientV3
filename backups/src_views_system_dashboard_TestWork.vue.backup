  ...
  <div>
    ...
    <div class="multi-tab-test">
        ...
        <template v-slot:title>
          <div>
            <a-form :form="form" layout="inline">
              <a-form-item label="自定义名:>
                <a-input" v-decorator="['tabName', {rules: [{required: true, message: '请输入新:Tab 名称'}]}]"></a>
              </a-form-item>
            </a-form>
          </div>
        </template>
      ...
      <a-popconfirm :open="visible2" @confirm="confirm2" @cancel="() => visible2 = false" okText="确定" cancelText="取消">
        <template v-slot:title>
          <div>
            <p>页面 KEY 是由页面的路:<code>path</code> 决定</p>
            <p>如果要修改某一个页面标题,该页面必须已经被打开:Tab </p>
            <p>后期可以考虑优化到编程式 Tab 栏,就可以没有这种限</p>
            <a-form :form="form2" layout="inline">
              <a-form-item label="页面KEY">
                <a-input v-decorator="['tabKey', { initialValue: '/dashboard/workplace' }]" ></a>
              </a-form-item>
              <a-form-item label="自定义名:>
                <a-input" v-decorator="['tabName', {rules: [{required: true, message: '请输入新:Tab 名称'}]}]"></a>
              </a-form-item>
